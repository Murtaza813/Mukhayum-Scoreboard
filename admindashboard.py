# admindashboard.py - NO SAMPLE DATA VERSION
...
try:
    ...
    st.error(f"❌ Error importing data functions: {e}")
...
with st.sidebar:
    ...
    if not IMPORT_SUCCESS:
        st.error("❌ Data functions not loaded")
    else:
        try:
            ...
            if not team_df.empty:
                ...
                st.caption("**Click Refresh button** to update")
            else:
...
with tab1:
    ...
    if not IMPORT_SUCCESS:
        st.error("❌ Data functions not available. Check your data_loader.py file.")
    else:
        try:
            team_df = get_team_data()
            ...
            if not team_df.empty:
                ...
                st.error("❌ No team data found in Google Sheets. Check if the data exists in the OFFICE WORKING sheet.")
...
with tab2:
    ...
    if not IMPORT_SUCCESS:
        st.error("❌ Data functions not available.")
    else:
        try:
            ...
            if not weekly_df.empty:
                ...
                if all(w in pivot_df.columns for w in week_order):
                ...
                st.error("❌ No weekly data found in Google Sheets. Check the Points Table Monthly sheet.")
